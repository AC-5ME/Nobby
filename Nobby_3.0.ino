...
void ENL() {
  ...
  //bool motor_running = false;        //motor running?
  char buf[10];   //buffer to store ENL digits
  char buf1[10];  //buffer for ENL time

  auto t = M5Dial.Touch.getDetail();

  snprintf(buf1, sizeof(buf1), " %-2dM %-2dS ", minutes, seconds);      //print ENL timer
  M5Dial.Display.setTextSize(1);
  M5Dial.Display.setTextFont(4);
  M5Dial.Display.setTextColor(GREEN, TIMER_bkcolor);
  M5Dial.Display.drawString(buf1, sx, sy - 50);

  if ((millis() - lastDisplayTime) > 1000) {  //ENL measurement delay
    for (int i = 0; i < 500; ++i) {           //Average samples
     ENL_value = map(analogRead(ENL_pin), 0, 4095, 0, 100);
      ENL_min = min(ENL_min, ENL_value);
      ENL_max = max(ENL_max, ENL_value);
    }

    int ENL_value = (ENL_max - ENL_min);  //measure noise level

    snprintf(buf, sizeof(buf), " %-2i%%", ENL_value);  //print ENL
    ...
    if (ENL_value > ENL_threshold) {
      unsigned long currentMillis = millis();  //Get the current time and start counter
      ...
      M5Dial.Display.setTextSize(1);
      M5Dial.Display.setTextFont(4);
      M5Dial.Display.setTextColor(RED, TIMER_bkcolor);
