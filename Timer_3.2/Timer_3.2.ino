...
static m5::touch_state_t prev_state;      //detect touch?
...
void loop() {
  ...
  if (prev_state != t.state && t.state == 9) {        //swipe?
        prev_state = t.state;
    switch (currentState) {
...
void cool_down() {
  ...
  if (M5Dial.BtnA.isPressed()) {  //Start timer
    delay(100);
...
void compass() {
  ...
  M5Dial.Display.setTextColor(WHITE, timer_bkcolor);
  M5Dial.Display.setTextSize(1);
  //M5Dial.Display.print((mag.magnetic.x), sx, sy - 40);
