...
// Beacon
#define SC16IS752_ADDR 0x4D  // I2C address for CJMCU-752 bridge
...
void sendUcpFrame(uint8_t type, const uint8_t* payload, uint8_t payloadLen) {
  ...
  ExtSerialA.write((uint8_t*)&frame, totalLen);
}
...
void parseUcpInput() {
  // Wait for SOM
  while (ExtSerialA.available() && ExtSerialA.read() != UCP_SOM)
    ;
  if (ExtSerialA.available() < 2) return;
  uint8_t len = ExtSerialA.read();
  if (ExtSerialA.available() < len + 3) return;  // type+payload+crc+eom

  uint8_t type = ExtSerialA.read();
  uint8_t payload[UCP_MAX_PAYLOAD] = { 0 };
  for (int i = 0; i < len - 1; i++) payload[i] = ExtSerialA.read();
  uint8_t crc_lo = ExtSerialA.read();
  uint8_t crc_hi = ExtSerialA.read();
  uint8_t eom = ExtSerialA.read();
...
void setup() {
 ...
 // Pass the I2C address to begin_i2c()
  if (!ExtSerialA.begin_i2c(SC16IS752_ADDR)) {
    Serial.println("SC16IS752 not found!");
  }
  Serial.println("SC16IS752 ready!");

  ExtSerialA.setFIFO(true);
  ExtSerialA.setBaudrate(115200);
  ExtSerialA.setLine(8, 0, 1); // 8N1
  requestStatus();
  ...
  // Only create big sprites once to conserve RAM
  if (!spriteAccel) {
    spriteAccel = new LGFX_Sprite(&M5Dial.Display);
    spriteAccel->setColorDepth(8);  // 1 byte per pixel conserves RAM
...
void COMPASS() {
  ...
  // Calibration method N, NE, E, SE, S, SW, W, NW
  if (M5Dial.BtnA.isPressed()) {
    delay(200);
