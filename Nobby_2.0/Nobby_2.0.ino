  ...
  if (t.state == 9) {  //swipe?
    ...
    switch (currentState) {
      ...
      case FUNCTION_THREE:  //beacon
        ...
        M5Dial.Display.setTextColor(WHITE, timer_bkcolor);
        M5Dial.Display.setTextFont(4);
        M5Dial.Display.drawString("VIB", sx, sy - 50);
...
void vib() {
  ...
  char buf[6];           //buffer to store vib digits
  int vib_samples = 10;  //number to average
  int x, y, z;           //vib samples
  int px, py, pz;        // previous vib measurement
  int vib_intensity = 0;
  ...
  // if ((millis() - lastDisplayTime) > 10) {  //print vib delay
  ...
  Serial.println(cal_accelZ);
*/
  for (int i = 0; i < vib_samples; i++) {
    float accel_x = accelerometer[0];
    float accel_y = accelerometer[1];
    float accel_z = accelerometer[2];

    int cal_accelX = accel_x - accel_offsetX;  //Apply accel calibration offsets
    int cal_accelY = accel_y - accel_offsetY;
    int cal_accelZ = accel_z - accel_offsetZ;

    vib_intensity += abs(cal_accelX - px);
    vib_intensity += abs(cal_accelY - py);
    vib_intensity += abs(cal_accelZ - pz);
    px = x;
    py = y;
    pz = z;
  }

  //int vib = (sqrt(cal_accelX * cal_accelX + cal_accelY * cal_accelY + cal_accelZ * cal_accelZ)) / 9.806;

  sprintf(buf, "%03d", vib_intensity);
